...
DBuilder.prototype.setPathToTemplates = function setPathToTemplates (pathToTemplates)
{
    ...
    if (typeof pathToTemplates !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип пути к директории шаблонов (ожидается: "string", принято: "' + typeof pathToTemplates + '").');
    if (!pathToTemplates.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к директории шаблонов.');
    if (!fsUtil.existsSync(pathToTemplates) || !$fs.statSync(pathToTemplates).isDirectory())
        throw new $swiftErrors.ValueError('Директория "' + pathToTemplates + '" не найдена.');
    //
    // задание пути к директории с шаблонами
    //
...
DBuilder.prototype.setDestination = function setDestination (destination)
{
    ...
    if (typeof destination !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип пути к директории назначения (ожидается: "string", принято: "' + typeof destination + '").');
    if (!destination.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к директории назначения.');
    if (!fsUtil.existsSync(destination) || !$fs.statSync(destination).isDirectory())
        throw new $swiftErrors.ValueError('Директория "' + destination + '" не найдена.');
    //
    // задание пути к директории назначения
    //
...
DBuilder.prototype.setStruct = function setStruct (struct)
{
    if (!typeUtil.isObject(struct))
        throw new $swiftErrors.TypeError('Недопустимый тип структуры директорий (ожидается: "object", принято: "' + typeof struct + '").');
    //
    // задание структуры директорий
    //
    this._struct = struct;
...
DBuilder.prototype.build = function build ()
{
    ...
    if (!self._struct)
        throw new $swiftErrors.SystemError('Не задана структура директорий.');
    if (!self._destination)
        throw new $swiftErrors.SystemError('Не задан путь к директории назначения.');
    ...
    {
        throw new $swiftErrors.SystemError('Ошибка создания струтктуры директорий.').setInfo(err);
    }
